...
template<>
std::list<int> read_one<std::list<int>>(std::istream& in) {
    int k;
...
template<typename T>
void loadVector(std::istream& in, Vector<T>& vec) {
    int n;
...
template<typename T>
void write(const Vector<T>& vec, std::ostream& out = std::cout) {
